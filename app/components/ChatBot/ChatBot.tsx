...
const ChatBot = ({ userId }: ChatBotProps) => {
  ...
  const getChatSession = async () => {
    try {
      const result = await fetch(
        `${process.env.NEXT_PUBLIC_CHATBOT_IP}/sessions`,
        {
          method: "POST",
          body: JSON.stringify({
            user_id: userId,
  ...
  const getChatHistory = async (chatSessionId: string) => {
    try {
      const result = await fetch(
        `${process.env.NEXT_PUBLIC_CHATBOT_IP}/chat-history`,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  const handleSendMessage = async (newInput = input) => {
    ...
    const apiResponse = await fetch(
      `${process.env.NEXT_PUBLIC_CHATBOT_IP}/message`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="bg-green-primary h-full flex flex-col items-center justify-center py-10 sm:py-20 px-3">
      <div className="max-w-[600px] w-full flex flex-col">
        {initialLoading ? (
          <Box textAlign="center" sx={{ pb: "2rem" }}>
          ...
          <>
            {messages &&
              ...
              messages.map((m: Message, index) => (
                <div
                  key={`${m.role}-${index}`}
                  className={`sm:max-w-70 mb-6 ${
                    m.role === "user"
                      ? "self-end bg-blueGrey-dark rounded-2xl p-3 text-right"
                      : "self-start flex gap-4"
                  }`}
                >
                  {m.role !== "user" && (
                    <Avatar
            ...
            >
              ...
              >
                <g
                  ...
                  strokeWidth="2"
                >
                  <path d="M6 18h8M3 22h18M14 22a7 7 0 1 0 0-14h-1M9 14h2M9 12a2 2 0 0 1-2-2V6h6v4a2 2 0 0 1-2 2H9ZM12 6V3a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v3" />
                </g>
              </svg>
